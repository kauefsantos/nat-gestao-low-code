...
test("venda pendente separa faturamento de caixa",()=>{const current=state([sale()]);const numbers=dashboardNumbers(current);assert.equal(saleValue(current.sales[0]),20);assert.equal(numbers.billed,20);assert.equal(numbers.revenue,0);assert.equal(numbers.receivedCash,0);assert.equal(numbers.receivables,20);assert.equal(numbers.orderCount,1);assert.equal(numbers.paidOrders,0);assert.equal(numbers.pendingOrders,1);assert.equal(numbers.cashIn,0);assert.equal(numbers.contribution,12);});
