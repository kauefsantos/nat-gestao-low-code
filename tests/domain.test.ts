...
import { buildSale, dashboardNumbers, productCost, supplyUsageCost, supplyUnitCost, type NatState, type Product, type Supply } from "../src/domain/nat.js";
import { familyPricingSummary, roundUpToHalf } from "../src/domain/catalog.js";
...
test("preço comum usa o sabor mais caro e arredonda para R$ 0,50", () => {
  ...
  const state:NatState={version:3,supplies:[chocolate],products,sales:[],expenses:[],settings:{ownerName:"Natalia",monthlyFixedCosts:0,paymentFeePercent:0,defaultMinimumMarginPercent:35,defaultTargetMarginPercent:50}};
...
test("gasto esporádico reduz o resultado do mês sem alterar contribuição da venda", () => {
  ...
  const state:NatState={version:3,supplies:[],products:[],sales:[{id:"s",productId:"p",productName:"Teste",quantity:1,totalReceived:20,paymentMethod:"pix",soldAt:`${date}T12:00:00Z`,unitCostSnapshot:5,variableFeeSnapshot:0,contributionSnapshot:15}],expenses:[{id:"e",name:"Cortador",amount:4,spentAt:date}],settings:{ownerName:"Natalia",monthlyFixedCosts:3,paymentFeePercent:0,defaultMinimumMarginPercent:35,defaultTargetMarginPercent:50}};
