...
-- Reconcile integrity rules already present in the Lovable Cloud runtime with the
-- canonical migration history. The migration is idempotent and fail-closed: it never
-- deletes valid business data to make a constraint fit.
...
begin
  ...
  ) then raise exception 'Integrity preflight failed: cross-business inventory cost allocation exists.'; end if;
  ...
  ) then raise exception 'Integrity preflight failed: cross-business recipe import lineage exists.'; end if;

  if exists (
    select 1 from public.recipe_items
    group by business_id,product_id,supply_id having count(*)>1
  ) then raise exception 'Integrity preflight failed: duplicated supply in recipe exists.'; end if;
  ...
  ) then raise exception 'Integrity preflight failed: notification/subscription ownership mismatch exists.'; end if;

  if exists (
    select 1 from public.customers
    where phone is not null and regexp_replace(phone,'\D','','g')<>''
    group by business_id,regexp_replace(phone,'\D','','g') having count(*)>1
  ) then raise exception 'Integrity preflight failed: duplicated normalized customer phone exists.'; end if;

  if exists (
    select 1 from public.customers
    where instagram is not null and regexp_replace(lower(btrim(instagram)),'^@','','')<>''
    group by business_id,regexp_replace(lower(btrim(instagram)),'^@','','') having count(*)>1
  ) then raise exception 'Integrity preflight failed: duplicated normalized customer Instagram exists.'; end if;
end $$;

-- Inventory cost allocations must stay inside the business that owns the cost layer.
do $$
begin
  if not exists (
...
-- Recipe-import lineage keeps product ownership aligned while preserving import history.
...
-- One supply may appear only once in the same recipe.
...
$$;
revoke all on function private.prevent_duplicate_recipe_supply() from public,anon,authenticated;
...
-- Delivery logs must reference a subscription belonging to the same business and user.
do $$
begin
  if not exists (
    select 1 from pg_constraint
...
-- Reimports are legitimate; this is a lineage lookup index, not a uniqueness contract.
...
-- Prevent probable duplicate customer records inside one business.
...
begin
  ...
  ) then raise exception 'Já existe um cliente com este telefone neste negócio.' using errcode='23505'; end if;
...
$$;
revoke all on function private.prevent_duplicate_customer_contact() from public,anon,authenticated;
