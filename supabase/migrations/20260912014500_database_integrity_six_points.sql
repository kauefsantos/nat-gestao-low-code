...
-- -----------------------------------------------------------------------------
-- Preflight: abort rather than silently mutating conflicting production data.
-- -----------------------------------------------------------------------------
do $$
begin
  if exists (
...
-- -----------------------------------------------------------------------------
-- 1) Cost allocations: sale item and cost layer must belong to the same business.
-- -----------------------------------------------------------------------------
do $$
begin
  if not exists (
...
-- -----------------------------------------------------------------------------
-- 2) Recipe import lineage: product must belong to the same business.
-- Preserve lineage if a product is physically deleted by nulling only product_id first.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 3) Recipe items: the same supply can appear only once per product recipe.
-- Trigger gives a domain-friendly error; unique constraint is the race-safe backstop.
-- -----------------------------------------------------------------------------
...
alter table public.recipe_items
  add constraint recipe_items_business_product_supply_key
  unique (business_id,product_id,supply_id);

-- -----------------------------------------------------------------------------
-- 4) Notification delivery ownership: subscription, user and business must match.
-- -----------------------------------------------------------------------------
do $$
begin
  if not exists (
...
-- -----------------------------------------------------------------------------
-- 5) Recipe import lineage index: it is lineage/history, not deduplication.
-- Reimports are intentional; staged metadata remains one-shot through pending row deletion.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 6) Customers: prevent probable duplicates by normalized contact inside one business.
-- Display values remain untouched. Trigger provides friendly errors; indexes are race-safe.
-- -----------------------------------------------------------------------------
