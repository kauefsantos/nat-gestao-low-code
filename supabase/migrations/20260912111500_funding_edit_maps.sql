...
declare
  ...
  v_fixed numeric:=0;v_fixed_source text:='owner';v_supply_funding jsonb:='{}'::jsonb;v_expense_funding jsonb:='{}'::jsonb;
begin
  ...
  select coalesce(monthly_fixed_costs,0),coalesce(fixed_cost_funding_source,'owner') into v_fixed,v_fixed_source from public.business_settings where business_id=p_business_id;
  with latest as(
    select distinct on(supply_id) supply_id,funding_source from public.supply_purchases where business_id=p_business_id
    order by supply_id,purchased_at desc,created_at desc,id desc
  ) select coalesce(jsonb_object_agg(supply_id::text,funding_source),'{}'::jsonb) into v_supply_funding from latest;
  select coalesce(jsonb_object_agg(id::text,funding_source),'{}'::jsonb) into v_expense_funding from public.sporadic_expenses
    where business_id=p_business_id and spent_at>=p_month_start and spent_at<(p_month_start+interval '1 month')::date;
  v_month_owner:=v_month_owner_purchases+v_month_owner_expenses;v_month_business:=v_month_business_purchases+v_month_business_expenses;
  if v_fixed_source='owner' then v_month_owner:=v_month_owner+v_fixed;else v_month_business:=v_month_business+v_fixed;end if;
  return jsonb_build_object('ownerContributions',v_owner_contributions,'ownerWithdrawals',v_owner_withdrawals,
    'ownerFundedOutflows',v_owner_purchases+v_owner_expenses,'businessReinvestment',v_business_purchases+v_business_expenses,
    ...
    'fixedCostFundingSource',v_fixed_source,'supplyFunding',v_supply_funding,'expenseFunding',v_expense_funding);
end $$;
