...
as $$
declare
  v_push jsonb;v_jobs jsonb;v_ai jsonb;
begin
  ...
  from public.notification_delivery_log where business_id=p_business_id;
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'stale',case when job_name='nat-push-local-tick' then coalesce(last_started_at,'epoch'::timestamptz)<now()-interval '2 hours'
                 else coalesce(last_started_at,'epoch'::timestamptz)<now()-interval '15 minutes' end
