...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(mode==="set_product_stock"){
      ...
      if(result.error){console.error("nat-inventory-production set_product_stock_v2",{requestId,productId,code:result.error.code,message:result.error.message,details:result.error.details,hint:result.error.hint});const insufficientStock=/estoque insuficiente/i.test(result.error.message);const status=insufficientStock?409:/receita|unidade|rendimento|saldo/i.test(result.error.message)?422:/conflict/i.test(result.error.message)?409:400;return json(status,{error:"STOCK_TARGET_REJECTED",...(insufficientStock?{reason:"INSUFFICIENT_STOCK"}:{}),message:result.error.message,requestId},cors);}
