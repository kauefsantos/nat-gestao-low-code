...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),AI_TIMEOUT_MS);let openai:Response;
    try{openai=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({model:"gpt-5.6-luna",instructions,input:`Formato: ${format}\nPedido da usuária: ${prompt}`,max_output_tokens:700}),signal:controller.signal});}catch(error){if(error instanceof Error&&error.name==="AbortError"){await finishLog(admin,logId,startedAt,"timeout",{errorCode:"AI_TIMEOUT"});return new Response(JSON.stringify({error:"AI_TIMEOUT"}),{status:504,headers:{...cors,"Content-Type":"application/json"}});}throw error;}finally{clearTimeout(timeout);}
    const responseJson=await openai.json();const usage=usageOf(responseJson);if(!openai.ok){await finishLog(admin,logId,startedAt,"provider_error",{providerStatus:openai.status,usage,errorCode:"AI_PROVIDER_ERROR"});return new Response(JSON.stringify({error:"AI_PROVIDER_ERROR"}),{status:502,headers:{...cors,"Content-Type":"application/json"}});}
    try{const outputText=extractOutputText(responseJson);if(!outputText)throw new Error("AI returned no text.");const parsed=parseJsonText(outputText);const result={headline:clampText(parsed.headline,52),subheadline:clampText(parsed.subheadline,90),caption:clampText(parsed.caption,550),cta:clampText(parsed.cta,45),visual_direction:clampText(parsed.visual_direction,220)};if(!result.headline||!result.caption)throw new Error("AI response was incomplete.");await finishLog(admin,logId,startedAt,"succeeded",{providerStatus:openai.status,usage});return new Response(JSON.stringify(result),{headers:{...cors,"Content-Type":"application/json"}});}catch(error){console.error("AI output validation",error instanceof Error?error.message:"unknown");await finishLog(admin,logId,startedAt,"invalid_output",{providerStatus:openai.status,usage,errorCode:"AI_INVALID_OUTPUT"});return new Response(JSON.stringify({error:"AI_INVALID_OUTPUT"}),{status:502,headers:{...cors,"Content-Type":"application/json"}});}
