...
select ok(has_function_privilege('authenticated','public.apply_nat_transition_v4(uuid,uuid,jsonb)','EXECUTE'),'authenticated can execute current transition API v4');
...
select ok(not has_function_privilege('authenticated','public.save_sale_items_v3(uuid,uuid,jsonb,numeric,text,timestamptz,uuid,text)','EXECUTE'),'legacy sale v3 is internal only');
select ok(position('save_sale_items_v4' in pg_get_functiondef('public.save_sale_items_v3(uuid,uuid,jsonb,numeric,text,timestamptz,uuid,text)'::regprocedure))>0,'legacy sale v3 still delegates internally to current sale rules');
...
select ok(position('pg_advisory_xact_lock' in pg_get_functiondef('public.save_supply(uuid,uuid,text,text,numeric,text,numeric,date)'::regprocedure))>0,'internal save_supply serializes concurrent writes');
