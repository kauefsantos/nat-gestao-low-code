...
select ok(has_function_privilege('authenticated','public.save_sale_items_v4(uuid,uuid,jsonb,numeric,text,timestamptz,uuid,text,text,numeric,text,boolean)','execute'),'authenticated can execute v4 sale RPC');
...
select lives_ok($$select public.save_sale_items_v4('b1111111-1111-4111-8111-111111111111','b7777777-7777-4777-8777-777777777771','[{"productId":"b5555555-5555-4555-8555-555555555555","quantity":2}]'::jsonb,12,'pix',now(),null,'sale','street',2,null,false)$$,'P2 sale with delivery succeeds');
...
select throws_ok($$select public.save_sale_items_v4('b1111111-1111-4111-8111-111111111111','b7777777-7777-4777-8777-777777777772','[{"productId":"b5555555-5555-4555-8555-555555555555","quantity":2}]'::jsonb,1,'pix',now(),null,'sale','street',0,'Promo fictícia',false)$$,'22023','Esta venda fica abaixo do custo. Confirme conscientemente para continuar.','below-cost sale is rejected without override');
...
select lives_ok($$select public.save_sale_items_v4('b1111111-1111-4111-8111-111111111111','b7777777-7777-4777-8777-777777777773','[{"productId":"b5555555-5555-4555-8555-555555555555","quantity":2}]'::jsonb,1,'pix',now(),null,'sale','street',0,'Promo fictícia',true)$$,'below-cost sale can be consciously overridden');
...
select throws_ok($$select public.save_sale_items_v4('b1111111-1111-4111-8111-111111111111','b7777777-7777-4777-8777-777777777774','[{"productId":"b5555555-5555-4555-8555-555555555555","quantity":1.5}]'::jsonb,9,'pix',now(),null,'sale','other',0,null,false)$$,'22023','A quantidade de produtos acabados precisa ser inteira e maior que zero.','fractional finished-product quantity is rejected');
select throws_ok($$select public.save_sale_items_v4('b1111111-1111-4111-8111-111111111111','b7777777-7777-4777-8777-777777777775','[{"productId":"b5555555-5555-4555-8555-555555555555","quantity":1}]'::jsonb,0,'other',now(),null,'courtesy','other',1,null,false)$$,'22023','Movimentações sem venda não podem ter custo de entrega.','courtesy cannot carry delivery cost');
