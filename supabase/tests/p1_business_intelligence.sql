...
select ok(has_function_privilege('authenticated','public.save_owner_cash_movement(uuid,uuid,text,numeric,date,text)','execute'),'owner cash mutation uses authoritative RPC');
...
select lives_ok($$select public.save_owner_cash_movement('a1111111-1111-4111-8111-111111111111','a7777777-7777-4777-8777-777777777771','contribution',100,current_date,'Capital de teste')$$,'owner contribution can be registered');
select lives_ok($$select public.save_owner_cash_movement('a1111111-1111-4111-8111-111111111111','a7777777-7777-4777-8777-777777777772','withdrawal',20,current_date,'Retirada de teste')$$,'owner withdrawal can be registered');
...
select lives_ok($$select public.record_inventory_production('a1111111-1111-4111-8111-111111111111','a5555555-5555-4555-8555-555555555555',1,now(),'Lote P1')$$,'production succeeds under P1 rules');
...
select set_config('request.jwt.claim.sub','a2222222-2222-4222-8222-222222222222',true);
select set_config('request.jwt.claims','{"sub":"a2222222-2222-4222-8222-222222222222","role":"authenticated","aal":"aal2"}',true);
select lives_ok($$select public.save_sale_items_v3('a1111111-1111-4111-8111-111111111111','a8888888-8888-4888-8888-888888888881','[{"productId":"a5555555-5555-4555-8555-555555555555","quantity":2}]'::jsonb,20,'card',now(),null,'sale')$$,'card sale uses frozen production layer');
...
select lives_ok($$select public.cancel_sale('a1111111-1111-4111-8111-111111111111','a8888888-8888-4888-8888-888888888881','Cancelamento P1')$$,'sale cancellation succeeds');
...
select lives_ok($$select public.save_sale_items_v3('a1111111-1111-4111-8111-111111111111','a8888888-8888-4888-8888-888888888882','[{"productId":"a5555555-5555-4555-8555-555555555555","quantity":1}]'::jsonb,0,'other',now(),null,'courtesy')$$,'courtesy can leave stock without revenue');
