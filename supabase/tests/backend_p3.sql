...
select ok(has_function_privilege('authenticated','public.apply_nat_transition_v4(uuid,uuid,jsonb)','execute'),'authenticated can execute current idempotent transition RPC');
...
select ok((select p.prosecdef from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='save_calendar_event_v3'),'idempotent calendar save is SECURITY DEFINER');
...
select lives_ok(
  $$select public.apply_nat_transition_v4('cccccccc-cccc-4ccc-8ccc-cccccccccccc','cccccccc-0000-4000-8000-000000000001','[{"type":"save_sporadic_expense","expectedUpdatedAt":null,"payload":{"id":"cccccccc-0000-4000-8000-000000000010","name":"Taxa de teste","amount":12.5,"spentAt":"2026-09-10"}}]'::jsonb)$$,
...
select throws_ok(
  $$select public.apply_nat_transition_v4('cccccccc-cccc-4ccc-8ccc-cccccccccccc','cccccccc-0000-4000-8000-000000000001','[{"type":"save_sporadic_expense","expectedUpdatedAt":null,"payload":{"id":"cccccccc-0000-4000-8000-000000000011","name":"Payload diferente","amount":99,"spentAt":"2026-09-10"}}]'::jsonb)$$,
...
select lives_ok(
  $$select public.save_calendar_event_v3('cccccccc-cccc-4ccc-8ccc-cccccccccccc','cccccccc-2000-4000-8000-000000000001','cccccccc-0000-4000-8000-000000000020',null,'2026-09-20','15:30','delivery','Entrega teste','Levar caixa','WhatsApp','Entregar no prazo','planned',true)$$,
  'calendar event can be created with reminder enabled through idempotent API'
);
select lives_ok(
