...
insert into public.supply_purchases(id,business_id,supply_id,package_quantity,package_unit,package_price,purchased_at) values
 ...
 ('b4444444-4444-4444-8444-444444444442','b1111111-1111-4111-8111-111111111111','b3333333-3333-4333-8333-333333333332',100,'unit',50,current_date);
...
values ('b4444444-4444-4444-8444-444444444443','b1111111-1111-4111-8111-111111111111','b3333333-3333-4333-8333-333333333331',100,'g',100,current_date);
...
select is(
  round(((public.quote_sale_v1('b1111111-1111-4111-8111-111111111111','[{"productId":"b5555555-5555-4555-8555-555555555555","quantity":2}]'::jsonb,20,'card',now(),'sale',2)->>'totalCost')::numeric),2),
  7.20::numeric,
  'quote uses frozen FIFO production cost plus packaging instead of latest ingredient purchase'
);
select is(
  ((public.quote_sale_v1('b1111111-1111-4111-8111-111111111111','[{"productId":"b5555555-5555-4555-8555-555555555555","quantity":2}]'::jsonb,20,'card',now(),'sale',2)->>'variableFee')::numeric),
  0.60::numeric,
  'quote uses payment-method fee'
);
select is(
  round(((public.quote_sale_v1('b1111111-1111-4111-8111-111111111111','[{"productId":"b5555555-5555-4555-8555-555555555555","quantity":2}]'::jsonb,20,'card',now(),'sale',2)->>'contribution')::numeric),2),
  10.20::numeric,
...
select set_config('request.jwt.claims','{"sub":"b2222222-2222-4222-8222-222222222222","role":"authenticated","aal":"aal2"}',true);
select lives_ok(
  $$select public.quote_sale_v1('b1111111-1111-4111-8111-111111111111','[{"productId":"b5555555-5555-4555-8555-555555555555","quantity":1}]'::jsonb,0,'other',now(),'courtesy',0)$$,
  'paused product can still be quoted for a noncommercial stock exit'
);
select throws_ok(
  $$select public.quote_sale_v1('b1111111-1111-4111-8111-111111111111','[{"productId":"b5555555-5555-4555-8555-555555555555","quantity":1}]'::jsonb,10,'pix',now(),'sale',0)$$,
