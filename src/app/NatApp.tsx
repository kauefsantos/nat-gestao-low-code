...
import { HomeView, PortfolioView, PricingView, ProductsView, SalesView, type ProductTab } from "@/components/nat/Views";
...
export function NatApp({ view,onView }: { view:NatView; onView:(view:NatView)=>void }) {
  ...
  return <AppShell view={view} onView={onView} onSale={()=>setSheet({type:"sale"})} onSettings={()=>setSheet({type:"settings"})} onLogout={logout}>
    ...
    {view==="portfolio"&&<PortfolioView state={state} onConfigure={(preset)=>setSheet({type:"product",preset})} onEdit={(value)=>setSheet({type:"product",value})} onPricing={()=>onView("pricing")}/>} 
