...
export const Route = createRootRouteWithContext<RouterContext>()({
  beforeLoad: async () => {
    ...
    return { auth: { isAuthenticated: Boolean(session), user: session?.user ?? null, session, isLoading: false } as AuthState };
  },
  head: () => ({
    meta: [
      { charSet: "utf-8" },
...
function Root() {
  ...
  const auth = useAuth();
  useEffect(() => { void router.invalidate(); }, [auth.isAuthenticated, router]);
