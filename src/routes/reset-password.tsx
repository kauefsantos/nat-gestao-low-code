...
import { isAuthConfigured, onPasswordRecovery, signOut, updatePassword } from "@/data/auth-repository";
...
function ResetPasswordPage(){
  ...
  useEffect(()=>{
    ...
    let active=true;
    const unsubscribe=onPasswordRecovery(()=>{if(active)setReady(true);});
  ...
  async function submit(event:React.FormEvent){
    ...
    try{
      ...
      setIsError(true);
      setMessage("Não foi possível alterar a senha. Abra novamente o link de recuperação mais recente.");
    }finally{
      setLoading(false);
    }
  }
