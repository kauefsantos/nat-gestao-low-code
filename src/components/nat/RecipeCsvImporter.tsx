...
export function RecipeCsvImporter({ supplies,onImport,hasRecipe }: { supplies: Supply[]; onImport: (items: RecipeItem[]) => void; hasRecipe: boolean }) {
  ...
  async function apply() {
    ...
    try {
      await stageRecipeImport({sourceType:fileName?"file_csv":"pasted_csv",fileName,content:text,rowCount:parsed.items.length});
    } catch (error) {
      setErrors([error instanceof Error?error.message:"Não foi possível registrar a origem do CSV."]);
      return;
    }
    onImport(parsed.items); setMessage(`${parsed.items.length} item(ns) aplicados à receita com origem registrada.`);
  }
