...
function FamilySection({ title,items,state,monthMap,allMap,onConfigure,onEdit }: { title:string; items:CatalogItem[]; state:NatState; monthMap:Map<string,AnalyticsEntry>; allMap:Map<string,AnalyticsEntry>; onConfigure:(item:CatalogItem)=>void; onEdit:(product:Product)=>void }) {
  ...
  return <section className="nat-card overflow-hidden p-0">
    <div className="grid gap-4 bg-chocolate p-5 text-white sm:grid-cols-[1fr_auto] sm:items-center sm:p-6">
      <div>
        ...
        <p className="mt-2 text-sm text-white/70">{configured}/{items.length} sabores configurados</p>
      </div>
    ...
    <div className="divide-y divide-[#ead6cc]">
      {items.map((item)=>{
        ...
        return <div key={item.key} className="grid gap-3 p-4 sm:grid-cols-[1fr_96px_96px_auto] sm:items-center sm:p-5">
          <div className="flex min-w-0 items-start gap-3">
            <span aria-hidden="true" className="mt-2 h-2 w-2 shrink-0 rounded-full bg-rose"/>
            <div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-caramel">Subcategoria</p><h3 className="mt-1 text-base font-bold sm:text-lg">{item.flavor}</h3><p className="mt-1 text-xs text-caramel">{product?"Receita e preço configuráveis":"Ainda não configurado"}</p></div>
          </div>
          ...
          <button type="button" className="text-button justify-self-start sm:justify-self-end" onClick={()=>product?onEdit(product):onConfigure(item)}>{product?"Editar":"Configurar"}<ChevronRight size={15}/></button>
        </div>;
      })}
    </div>
  </section>;
...
export function PortfolioHierarchyView({ state,onConfigure,onEdit,onPricing }: { state:NatState; onConfigure:(item:CatalogItem)=>void; onEdit:(product:Product)=>void; onPricing:()=>void }) {
  ...
  return <section className="space-y-5">
    <PageHeading eyebrow="Categorias e sabores" title="Portfólio" text="Veja primeiro as duas grandes categorias da NAT e, logo abaixo, cada sabor como subcategoria. A quantidade vendida aparece na frente de cada nível para facilitar a leitura." action={<button type="button" className="secondary-button" onClick={onPricing}><TrendingUp size={18}/> Ver preços</button>}/>
    ...
    <FamilySection title="Brownies" items={BROWNIE_CATALOG} state={state} monthMap={monthMap} allMap={allMap} onConfigure={onConfigure} onEdit={onEdit}/>
    <FamilySection title="Brigadeiros" items={BRIGADEIRO_CATALOG} state={state} monthMap={monthMap} allMap={allMap} onConfigure={onConfigure} onEdit={onEdit}/>
  </section>;
}
