...
function CollapsiblePanel({title,text,icon,children,dark=false,eyebrow}:PanelProps){
  ...
  return <section className={dark?"rounded-[28px] bg-chocolate p-6 text-white sm:p-8":"nat-card"}>
    <div className="flex items-start justify-between gap-4">
      <div className="flex min-w-0 items-start gap-3">
        {icon&&<div className={`grid h-10 w-10 shrink-0 place-items-center rounded-full ${dark?"bg-white/10":"bg-rose-soft"}`}>{icon}</div>}
        <div className="min-w-0">
          {eyebrow&&<p className={`text-xs font-bold uppercase tracking-[.18em] ${dark?"text-rose":"text-caramel"}`}>{eyebrow}</p>}
          <h2 className={dark?"font-display text-3xl sm:text-4xl":"font-display text-3xl"}>{title}</h2>
          <p className={`mt-1 text-sm leading-6 ${dark?"text-white/75":"text-caramel"}`}>{text}</p>
        </div>
      </div>
      <button type="button" className={dark?"inline-flex min-h-10 shrink-0 items-center gap-2 rounded-xl border border-white/25 px-3 text-sm font-bold text-white":"secondary-button shrink-0"} aria-expanded={open} onClick={()=>setOpen((value)=>!value)}>{open?<ChevronUp size={17}/>:<ChevronDown size={17}/>} {open?"Retrair":"Expandir"}</button>
    </div>
