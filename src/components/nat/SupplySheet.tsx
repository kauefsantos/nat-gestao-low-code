...
export function SupplySheet({value,preset,recentSupplies=[],onClose,onSave}:{value?:Supply;preset?:StarterSupply;recentSupplies?:Supply[];onClose:()=>void;onSave:(supply:Supply)=>void}) {
  ...
  const [unit,setUnit]=useState<Unit>(value?.packageUnit??preset?.suggestedUnit??"unit");
  ...
  function submit(event:React.FormEvent) {
    ...
    if(!Number.isFinite(q)||q<=0){setError("A quantidade comprada precisa ser maior que zero.");return;}
    if(!Number.isFinite(p)||p<0){setError("O preço não pode ser negativo.");return;}
    if(!date){setError("Informe a data da compra.");return;}
    onSave({...preview,id:value?.id??id("supply"),name:name.trim()});
  }
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-[#35150A]/35 backdrop-blur-[2px]" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}>
    <div ref={panelRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="supply-sheet-title" className="h-full w-full max-w-xl overflow-y-auto bg-[#FFF9F6] p-5 pb-[max(24px,env(safe-area-inset-bottom))] pt-[max(20px,env(safe-area-inset-top))] shadow-2xl sm:p-7">
      ...
      <form className="mt-7 space-y-5" onSubmit={submit}>{latestPurchase&&<button type="button" className="secondary-button w-full justify-center" onClick={repeatLatest}>Repetir última compra: {latestPurchase.name}</button>}
        ...
        <div><label htmlFor="supply-price" className="field-label">Quanto você pagou?</label><div className="relative"><span className="absolute left-4 top-1/2 -translate-y-1/2 font-bold text-caramel">R$</span><input id="supply-price" className="nat-input pl-12" inputMode="decimal" value={price} onChange={(e)=>setPrice(e.target.value)} placeholder="12,00" required/></div></div>
        <div><label htmlFor="supply-date" className="field-label">Data da compra</label><input id="supply-date" type="date" className="nat-input" value={date} onChange={(e)=>setDate(e.target.value)} required/></div>
        {q>0&&p>=0&&<div className="rounded-2xl bg-rose-soft p-4"><p className="text-xs font-bold uppercase tracking-wider text-caramel">A NAT calculou</p><p className="mt-1 font-display text-2xl">Cada {unitLabel[preferredUsageUnit(unit)]} custa {money(supplyUnitCost(preview))}</p></div>}
        {error&&<p role="alert" className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p>}
        <button className="primary-button w-full justify-center" type="submit">Salvar compra</button>
      </form>
    </div>
  </div>;
}
