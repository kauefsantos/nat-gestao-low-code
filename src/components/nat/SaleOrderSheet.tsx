...
import { buildSaleOrder, money, paymentFeeForMethod, paymentLabel, transactionTypeLabel, type NatState, type PaymentMethod, type Sale, type TransactionType } from "@/domain/nat";
...
export function SaleOrderSheet({ state,onClose,onSave }: { state:NatState; onClose:()=>void; onSave:(sale:Sale)=>void }) {
  ...
  const paymentFee=paymentFeeForMethod(state.settings,method);
  ...
  try {
    ...
    if(items.length&&items.every((item)=>item.product&&item.quantity>0)) preview=buildSaleOrder({items,supplies:state.supplies,paymentFeePercent:paymentFee,totalReceived:received,paymentMethod:method,soldAt:`${date}T12:00:00.000Z`,customerId:customerId||null,transactionType});
  } catch { preview=null; }
  ...
  function submit(event:React.FormEvent) {
    ...
    try { onSave(buildSaleOrder({items:items.map((item)=>({product:item.product!,quantity:item.quantity})),supplies:state.supplies,paymentFeePercent:paymentFee,totalReceived:received,paymentMethod:method,soldAt:`${date}T12:00:00.000Z`,customerId:customerId||null,transactionType})); }
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-[#35150A]/35 backdrop-blur-[2px]" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}><div ref={panelRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="sale-order-title" className="h-full w-full max-w-xl overflow-y-auto bg-[#FFF9F6] p-5 pb-[max(24px,env(safe-area-inset-bottom))] pt-[max(20px,env(safe-area-inset-top))] shadow-2xl sm:p-7">
    ...
    <div className="mt-7">{sellable.length===0?<div className="rounded-2xl border border-dashed border-nat p-8 text-center text-sm text-caramel">Nenhum sabor disponível com receita completa.</div>:<form className="space-y-5" onSubmit={submit}>
      ...
      {transactionType==="sale"&&<><div className="rounded-2xl bg-rose-soft p-4"><div className="flex items-center justify-between gap-3"><span className="text-sm font-bold">Preço de tabela</span><strong className="font-display text-2xl">{money(listTotal)}</strong></div></div><div><label htmlFor="sale-total" className="field-label">Valor realmente recebido</label><input id="sale-total" className="nat-input" inputMode="decimal" value={total} onChange={(event)=>setTotal(event.target.value)}/>{hasAdjustment&&<p className="mt-2 text-sm text-caramel">{adjustment<0?"Desconto":"Acréscimo"}: <strong>{money(Math.abs(adjustment))}</strong></p>}</div><fieldset><legend className="field-label">Como recebeu?</legend><div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{(["pix","cash","card","other"] as PaymentMethod[]).map((item)=><button key={item} type="button" aria-pressed={method===item} className={`choice-button ${method===item?"active":""}`} onClick={()=>setMethod(item)}>{paymentLabel[item]}</button>)}</div>{paymentFee>0&&<p className="field-help">Taxa configurada para {paymentLabel[method]}: {paymentFee}%.</p>}</fieldset></>}
      ...
      {preview&&<div className="rounded-2xl bg-rose-soft p-4"><p className="text-xs font-bold uppercase tracking-wider text-caramel">Impacto desta movimentação</p><p className="mt-1 font-display text-3xl">{money(preview.contributionSnapshot)}</p><p className="mt-1 text-xs text-caramel">O banco recalcula o custo e congela a taxa do pagamento ao salvar.</p></div>}
