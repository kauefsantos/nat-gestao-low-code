...
export function supplyUsageCost(supply: Supply, quantity: number, unit: Unit) {
  ...
  if (packageBase.dimension !== usageBase.dimension || packageBase.value <= 0) return 0;
...
export function productCost(product: Product, supplies: Supply[], paymentFeePercent: number): ProductCost {
  ...
  const contributionAtCurrentPrice = product.sellingPrice - unitCost - variableFee;
  const marginAtCurrentPrice = product.sellingPrice > 0
...
export function dashboardNumbers(state: NatState) {
  ...
  for (const sale of sales) {
    const current = byProduct.get(sale.productId) ?? { name: sale.productName, quantity: 0 };
