...
import { initialHistoryCursor, loadNatHistoryPage, loadNatOperationalStateV2, type HistoryCursor } from "@/data/nat-operational-v2";
...
type LoadedState=Awaited<ReturnType<typeof loadNatOperationalStateV2>>;
...
export function useNatStore(){
  ...
  const reloadFromCloud=useCallback(async()=>{const loaded=await loadNatOperationalStateV2();applyLoaded(loaded);setReady(true);},[applyLoaded]);
  ...
  const refresh=useCallback(()=>{setReady(false);setLoadError(null);void reloadFromCloud().then(()=>{setInventoryRevision((value)=>value+1);setFundingRevision((value)=>value+1);}).catch((error)=>{devError("[NAT] Falha ao atualizar",error);setLoadError("Não foi possível atualizar os dados da NAT. Confira a conexão e tente novamente.");setReady(true);});},[reloadFromCloud]);
