...
export function useNatStore(){
  ...
  const handleWriteError=useCallback(async(error:unknown,sequence:number,fallback:string)=>{devError("[NAT] Falha ao persistir alteração",error);const classified=classifyApiError(error);authoritativeSupplyReload.current=false;try{if(businessIdRef.current)invalidateFundingSummary(businessIdRef.current);await reloadFromCloud();setInventoryRevision((value)=>value+1);setFundingRevision((value)=>value+1);}catch(reloadError){devError("[NAT] Falha ao recarregar após erro",reloadError);}if(sequence!==writeSequence.current)return;showSyncNotice({tone:"error",message:classified.code==="INTERNAL_ERROR"?fallback:classified.message});},[reloadFromCloud,showSyncNotice]);

  const update=useCallback((recipe:(current:NatState)=>NatState,success?:NatWriteSuccess,context:PersistContext={})=>{const previous=stateRef.current;const next=recipe(previous);const businessId=businessIdRef.current;applyState(next);if(!businessId)return;const sequence=++writeSequence.current;showSyncNotice({tone:"saving",message:"Salvando alteração..."});writeQueue.current=writeQueue.current.then(async()=>{const result=await persistNatTransition(businessId,previous,next,versionsRef.current,context);versionsRef.current=mergeVersions(versionsRef.current,result.versions);if(result.inventoryChanged)setInventoryRevision((value)=>value+1);if(result.fundingChanged){invalidateFundingSummary(businessId);setFundingRevision((value)=>value+1);}if(result.supplyChanged)authoritativeSupplyReload.current=true;if(sequence===writeSequence.current&&authoritativeSupplyReload.current){authoritativeSupplyReload.current=false;invalidateFundingSummary(businessId);await reloadFromCloud();setInventoryRevision((value)=>value+1);setFundingRevision((value)=>value+1);}if(sequence===writeSequence.current)showSyncNotice({tone:"saved",message:success?.message??"Tudo salvo.",actionLabel:success?.actionLabel,onAction:success?.onAction},!success?.actionLabel);}).catch((error)=>handleWriteError(error,sequence,"A alteração não pôde ser salva com segurança e foi desfeita. Confira os campos e tente novamente."));},[applyState,handleWriteError,reloadFromCloud,showSyncNotice]);
