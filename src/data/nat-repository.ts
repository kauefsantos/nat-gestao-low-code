...
export async function persistNatTransition(businessId:string,previous:NatState,next:NatState,versions:NatVersions,context:PersistContext={}):Promise<TransitionResult>{
  ...
  for(let attempt=0;attempt<2;attempt+=1){const result=await supabase.rpc("apply_nat_transition_v3",payload);if(!result.error){data=result.data;lastError=null;break;}lastError=result.error;const message=result.error.message??"";if(!/fetch|network|timeout|Failed to fetch/i.test(message))break;}
