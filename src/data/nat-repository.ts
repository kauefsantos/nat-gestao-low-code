...
export async function setProductAvailabilityCloud(businessId:string,productId:string,expectedUpdatedAt:string|null,available:boolean):Promise<string|null>{
  const result=await supabase.rpc("set_product_availability" as never,{p_business_id:businessId,p_id:productId,p_expected_updated_at:expectedUpdatedAt,p_available:available} as never);failure("Não foi possível alterar a disponibilidade do produto",result.error);
...
export async function persistNatTransition(businessId:string,previous:NatState,next:NatState,versions:NatVersions,context:PersistContext={}):Promise<TransitionResult>{
  ...
  for(const sale of next.sales)if(!previousSales.has(sale.id))operations.push({type:"create_sale",expectedUpdatedAt:null,payload:{id:sale.id,items:sale.items.map((item)=>({productId:item.productId,quantity:item.quantity})),totalReceived:sale.totalReceived,paymentMethod:sale.paymentMethod,soldAt:sale.soldAt,customerId:sale.customerId??null,transactionType:sale.transactionType??"sale",saleChannel:sale.saleChannel??"other",deliveryCost:sale.deliveryCostSnapshot??0,discountReason:sale.discountReason??null,belowCostOverride:Boolean(sale.belowCostOverride)}});
  ...
  for(const expense of next.expenses){const old=previousExpenses.get(expense.id);if(!old||!same(old,expense))operations.push({type:"save_sporadic_expense",expectedUpdatedAt:old?versions.expenses[expense.id]??null:null,payload:{id:expense.id,name:expense.name,amount:expense.amount,spentAt:expense.spentAt.slice(0,10),fundingSource:context.fundingSource??"business"}});}
  ...
  for(let attempt=0;attempt<2;attempt+=1){const result=await supabase.rpc("apply_nat_transition_v3" as never,payload as never);if(!result.error){data=result.data;lastError=null;break;}lastError=result.error;const message=result.error.message??"";if(!/fetch|network|timeout|Failed to fetch/i.test(message))break;}
