...
export type PersistContext={fundingSource?:FundingSource;fixedCostFundingSource?:FundingSource;equivalentToSupplyId?:string|null;appendSupplyPurchase?:boolean};
export type TransitionResult={versions:Partial<NatVersions>;inventoryChanged:boolean;fundingChanged:boolean;supplyChanged:boolean};
...
export async function persistNatTransition(businessId:string,previous:NatState,next:NatState,versions:NatVersions,context:PersistContext={}):Promise<TransitionResult>{
  ...
  if(!operations.length)return{versions:{},inventoryChanged:false,fundingChanged:false,supplyChanged:false};
  ...
  const supplyChanged=operations.some((op)=>op.type==="save_supply"||op.type==="delete_supply");
  return{versions:{settings:response.settings??undefined,supplies:cleanVersionMap(response.supplies),products:cleanVersionMap(response.products),sales:cleanVersionMap(response.sales),expenses:cleanVersionMap(response.expenses)},inventoryChanged:Boolean(response.inventoryChanged),fundingChanged,supplyChanged};
}
