...
type ReceivableRow={id:string;sale_value_snapshot:number|string;payment_status:string;payment_promised_date:string|null;payment_promised_time:string|null;payment_due_at:string|null;paid_at:string|null;payment_critical_at:string|null};
...
function mergeReceivable(sale:Sale,row:ReceivableRow|undefined):Sale{
  ...
  return{...sale,saleValueSnapshot:numberValue(row.sale_value_snapshot),paymentStatus:(row.payment_status==="pending"?"pending":"paid") as PaymentStatus,paymentPromisedDate:row.payment_promised_date,paymentPromisedTime:row.payment_promised_time,paymentDueAt:row.payment_due_at,paidAt:row.paid_at,paymentCriticalAt:row.payment_critical_at};
}
