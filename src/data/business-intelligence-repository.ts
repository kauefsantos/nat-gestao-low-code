...
export type BiCustomerSegment="Novo"|"Recorrente"|"VIP"|"Em risco"|"Inativo"|"Base insuficiente";
export type BiCustomer={customerId:string;name:string;source:string|null;firstPurchase:string;lastPurchase:string;orders:number;billed:number;received:number;averageTicket:number;units:number;daysSinceLast:number;recurring:boolean;favoriteProduct:string|null;nonCommercialInteractions:number;contribution:number;marginPercent:number;recencyScore:number;frequencyScore:number;valueScore:number;rfmTotal:number;segment:BiCustomerSegment};
...
async function fetchSnapshot(businessId:string):Promise<BusinessIntelligenceSnapshot>{
  ...
  return{
    ...
    products:rows(root.products).map(parseProduct),channels:rows(root.channels).map(parseChannel),customers,origins:rows(root.origins).map(parseOrigin),
